...
9. **Per-state continuing-education / licensing overlay (auto-pulled when `config.licensing_state` is set)** — The skill must overlay state-specific CE-hour requirements, license-renewal cycles, and state-only certifications on the 90-day plan. Examples: California (CSLB C-20 contractor license — renews every 2 years and imposes **no continuing-education requirement**; do not schedule state CE hours for a California hire — the overlay here is EPA 608 currency, manufacturer certs, and Cal/OSHA safety, not a CSLB CE clock), Texas (TDLR Air Conditioning and Refrigeration Contractor License — 8 CE hours/year), Florida (DBPR Class A/B/CAC — 14 CE hours per renewal cycle), New York (varies by jurisdiction, NYC DOB Refrigeration — 7 CE hours/year), Washington (L&I HVAC/R Specialty — 24 hours/3-year cycle), Maryland (HVACR contractor — 16 hours/2-year cycle). **Not every state has a CE clock — several (California among them) require none; the overlay must not invent one.** When `config.licensing_state` is set, surface the state's actual CE clock + the next renewal due date for this hire (computed from `start_date` + state cycle length) so the curriculum schedules CE hours instead of leaving them to lapse. If `config.licensing_state` is not set, the skill emits a single line at the top of the plan: "[STATE LICENSING — confirm renewal cycle and CE hours; set config.licensing_state to auto-schedule]" and proceeds with the federal / DOL / EPA defaults.
...
Example: a Texas TDLR ACR-licensed hire starting 2026-05-12 (TDLR license renews annually and requires 8 CE hours per year) has its next renewal ~12 months out; the plan schedules ~4 hours of company-sponsored CE in Q3 2026 and ~4 hours in Q1 2027 so the 8-hour annual requirement is met well before renewal, not in a scramble. (Contrast California: a CSLB C-20 hire gets **no** CE line scheduled — CSLB has no CE requirement — only the 2-year renewal reminder and EPA 608 / manufacturer-cert currency.)
